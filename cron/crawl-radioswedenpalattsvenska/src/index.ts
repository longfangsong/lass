...
async function fetch_single_article(title: string, url: string, db: D1Database): Promise<void> {
	...
	const update_time = new Date($('time.publication-metadata__item').attr('datetime') || '');
	const id = crypto.randomUUID();
	const stmt = db.prepare('INSERT INTO Article (id, title, content, update_time, url, voice_url) VALUES (?1, ?2, ?3, ?4, ?5, ?6);');
	await stmt.bind(id, title, content, update_time.getTime(), url, audioUrl).run();
}
