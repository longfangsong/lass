...
export async function searchWord(
  ...
  const directMatchTask = wordTable.where("lemma").equals(spell).toArray();

  // words which is a form of the word "spell"
  const formMatchTask = wordIndexTable
    .where("spell")
    .equals(spell)
    .filter((index) => index.form !== null)
    .toArray()
    .then((indexes) =>
      Promise.all(indexes.map((index) => wordTable.get(index.word_id))),
    );

  // words which `lemma` start with "spell"
  const likeMatchTask = wordTable.where("lemma").startsWith(spell).toArray();

  // words which has an index which starts in "spell"
  const likeFormMatchIndexesTask = wordIndexTable
    .where("spell")
    .startsWith(spell)
    .toArray()
    .then((indexes) => {
  ...
  const likeFormMatchTask = (async () => {
    ...
    const words = await wordTable.bulkGet(word_ids);
    return words;
  })();
  ...
  const result = await Promise.all(
    words.map(async (word) => {
      const lexemes = await lexemeTable
        .where("word_id")
        .equals(word!.id)
        .toArray();
      return {
